...
for the MY_DREGON dataset, it is in a different folder because 'gender' and 'speaker' do not matter
now its 'secondary_type' and 'primery_type' that matter
"""

# TODO: implement check_valid_types
# TODO: check and update all of the functions
...
class ArtificialDatasetCreator(object):
    ...
    def __init__(self,
        ...
        if audio_dataset_name.lower() == "my_dregon":
            self.data_loader = my_dregon_loader.MyDregonLoader()
            self.fs = 16000
        else:
...
def check_valid_types(types):
    """checks that the types given are of a valid form and exist in the dataset"""
    return True
...
class RandomCombinations(ArtificialDatasetCreator):
    def __init__(self,
        ...
        self.types = types
        valid_types = check_valid_types(self.types)
        assert valid_types, ('not a valid combination of types')

        # self.used_speakers = self.get_available_speakers(
        #                          subset_of_speakers)
        ## this parts finds al of the speakers that are relevant in the dataset.
        ## this does not help us here for having only the primerey and secondary types.
        ...
        # else:
        #     self.val_speakers = []
        ## this part is about creating the validation set based on the number of valid speakers

        # self.used_speakers = [s for s in self.used_speakers
        #                       if s not in self.val_speakers]
        ## and this part is about removing the samples used for validation from the set of used ones
        ## TODO: changd the above 2 ones
    ...
    ## TODO: figure out what to do with this function / how to change it

    @staticmethod
    def random_combinations(iterable, r):
        """this generates every possible outcome from choosing r from the set,
    ...
    def get_only_valid_mixture_combinations(self,
        ...
        """generates mixtures of n_mixed_sources, out of the possible sources,
        if n_mixture is 0 or less, returns the number of possible outcomes
        @returns a list of valid combinations(lists of n_mixed_sources, with source_information)""" 
        ...
        #         continue
        ## this is dealing with having only the correct genders and not having the same speaker twice in the combination
        ## TODO: figure out what to do here instead
            # check whether all the signals have the appropriate
            # duration
        ## source info is the extra data to specify the exact sample, the speaker id and the sentance id.
        ## figure out how to change that implementation it
            signals = [(len(self.get_wav(speakers_dic, source_info))
                        >= self.min_samples + convolution_offset)
                       for source_info in possible_comb]
            if not all(signals):
                continue
        ## this part makes sure that all of the data samples are long enough.
        ## TODO: figure out how to change the way that the dataset is created so the snippits are long enough,
        ##       and why do we need convolution_offset
    ...
    ## TODO: change this so this will work for our dataset
    ...
    ## TODO: change this so it will work for our dataset
    ...
    ## the function basicly takes the 3 objects and merges them into one
    ## TODO: change only the documentation here, no need to change anithing about the implementation
    ...
    def gather_mixtures_information(self,
        ...
        ## this creates the random positions, and mixes them all together
        ## TODO: change this so it will be able to work with the new configurations, of the dregon dataset
    ...
    def update_label_masks_and_info(self,
        ...
        ## TODO: make sure that construct_mixture works with mixture_info, that might be changed
    ...
    ## this also does not seems to have to change
    ## TODO: only change the 'speaker' to the correct form
    ...
    def get_all_mixture_sets(self,
        ...
        for speaker_set, speakers in speaker_sets.items():
            mixtures[speaker_set] = self.get_mixture_combinations(
                                    ...
                                    get_only_ground_truth)
        return mixtures
    ## TODO: just change the speaker and the other thing in there to work as needed
